    ...
    <div className="space-y-12">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <ChartCard title={`Devlet Üniversiteleri — En Sık Kelimeler (${tab.charAt(0).toUpperCase() + tab.slice(1)})`}>
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={devletWords.slice(0, 15)} layout="vertical" margin={{ left: 100, right: 30 }}>
              <XAxis type="number" tick={{ fontSize: 11 }} />
              <YAxis type="category" dataKey="word" tick={{ fontSize: 11, fill: '#94A3B8' }} width={100} />
        ...
        <ChartCard title={`Vakıf Üniversiteleri — En Sık Kelimeler (${tab.charAt(0).toUpperCase() + tab.slice(1)})`}>
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={vakifWords.slice(0, 15)} layout="vertical" margin={{ left: 100, right: 30 }}>
              <XAxis type="number" tick={{ fontSize: 11 }} />
              <YAxis type="category" dataKey="word" tick={{ fontSize: 11, fill: '#94A3B8' }} width={100} />
              <Tooltip contentStyle={{ background: '#1E293B', border: '1px solid #334155', borderRadius: 8 }} />
      ...
      <ChartCard title="Devlet vs Vakıf — Kelime Frekansı Karşılaştırması">
        <ResponsiveContainer width="100%" height={440}>
          <BarChart data={compareData} layout="vertical" margin={{ left: 120, right: 30 }}>
            <XAxis type="number" tick={{ fontSize: 11 }} />
            <YAxis type="category" dataKey="word" tick={{ fontSize: 11, fill: '#94A3B8' }} width={120} />
            <Tooltip contentStyle={{ background: '#1E293B', border: '1px solid #334155', borderRadius: 8 }} />
